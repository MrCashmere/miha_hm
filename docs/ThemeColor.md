...
这个开关对外承诺的就四件事，下面每一节分别对应其中一件：

| 行为 | 落在哪 |
| --- | --- |
| **跟手**：手指滑到哪、光落在哪，按住滚动也跟着走 | §5.9.1 的坐标换算 |
| **主题色**：光的颜色 = 当前主题色，关掉开关回到系统白光 | §5.9.1 / §5.9.2 |
| **只在卡片内**：光不溢到相邻卡片上 | §5.9.1 的分层与裁切 |
| **底栏同步**：底部 HdsTab 悬浮栏的流光同时换成主题色 | §5.9.2 |

另有一个**「卡片流光触感」**开关（`cardGlowTouch`，**默认关闭**）：

- **关**（默认）：**按压卡片不出现任何流光**；
- **开**：按压卡片出现完整的跟手流光（跟手光斑 + 溢出到卡间缝隙的环境光）。
...
private spillVisible(): boolean {       // 环境光层
  return this.glowVisible() && this.glowTouchEnabled;
}
private touchGlowVisible(): boolean {   // 跟手光斑层
  return this.glowVisible() && this.glowTouchEnabled;
}
```

> ⚠️ **不要想着「只关跟手光斑、留着环境光」。** 试过，开关等于没生效：
> 那层环境光的圆心本来就是 `glowX/glowY`，它自己就是个跟手的光斑，
> 看起来仍是「触碰反馈的流光」。**两层要么一起亮，要么一起不亮**——
> 这也是这个开关最终被定义成「按压卡片出不出现流光」的原因。
...
| 溢出光晕 | 比卡片大一圈（0 尺寸锚点放置） | 最底层，卡内被材质盖住 | 只让卡外那部分漏出来，见下方的实测结论 |
...
- **「只在卡片内」靠的是这一层自己带 `borderRadius`。** 外壳为了放行溢出光晕
  已经不设 `clip(true)`（见 §7.9 附近的说明），所以裁切责任下移到每一层：
  光斑层给了 `borderRadius(this.cardRadius)`，光到圆角处就被收住，不会越出卡片。
- **溢出光晕刻意不抬 `zIndex`**：让后画的相邻卡片盖住它，只留卡外那圈。
  第一版做成 `shadow()` 外发光是错的——那是给卡片描了一圈边。

> ⚠️ **实测结论：这层溢出光晕到不了相邻卡片上。** 它只能让卡片之间的缝隙亮一点，
> 邻卡的玻璃不透明到足以把剩下的全盖掉。想让邻卡边缘真的反光，
> 得让每张卡自己画自己的边（见本节末「试过但已回退」）——
> 这一点在 1.1.3 的更新说明里对用户也是如实写明的。

```ts
@Builder
cardBody() {
  this.glowSpillLayer()          // ① 溢出光晕：最底层，卡内被材质盖住，只留卡外那圈
  if (this.materialActive()) {
    MaterialCardLayer({ layerRadius: this.cardRadius })   // ② 材质
  }
  if (this.cardTint.length > 0) { /* ③ 沾色层，见 §5.8；自带 borderRadius */ }
  this.pressGlowLayer()          // ④ 跟手光斑：材质之上、内容之下；自带 borderRadius
  Flex({ ... }) { this.cardContent() }                    // ⑤ 内容
}
```

#### 5.9.2 底部 HdsTab 悬浮栏的流光
...
#### 试过但已回退：让光「照到」相邻卡片的边缘

目标效果是：光从被按住的卡片溢出去，在**旁边卡片的边上**留下一小段高光（玻璃棱的反光）。
实现方式是「光源广播 + 各卡自绘」：被按住的卡把光源位置写进 AppStorage（`PressGlowBeacon`），
每张卡挂一个 `PressRim` 子组件，各自订阅该键、用 `onAreaChange` 量自己的矩形、
算出自己被照到的那一段并在边缘画一段渐变高光。

效果达到了（真机上能看到邻卡边缘亮起一小段，位置随手指走），但**掉帧严重，已整体回退**。
原因很清楚：`PressRim` 挂在**每一张** MaterialCard 上，等于每张卡都多了一个
AppStorage 订阅者 + 一个 `onAreaChange`；列表滚动时满屏卡片的 `onAreaChange` 接连触发、
`setState` 再各自重绘，滚动帧率直接塌掉。加上光源广播按 25Hz 写入 AppStorage，
一次按压会让所有卡片的 `PressRim` 在同一帧里排队重绘。

**要做这件事，先解决订阅面的问题**，可行方向（都未验证）：

- **只在有光时才挂** `PressRim`：没按压时一张都不挂，滚动零成本；
  光一出现才挂载、靠首次布局的 `onAreaChange` 拿到矩形。代价是每次按压都有一批组件挂载。
- **别让每张卡都订阅**：广播只写一个「当前光源 + 版本号」，
  由页面自己筛出光源附近的卡（页面知道各卡位置），只给这几张下发 `@Prop`。
- **不要用 `onAreaChange` 量矩形**：滚动时它是每个卡片实例各触发一次；
  改成从列表的滚动偏移 + 卡片在列表中的已知位置直接算，只有一次计算。

> 顺带记一条踩坑：`PressRim` 第一版把量尺寸的层写进了 `if (falloff > 0)` 里，
> 而 `falloff` 又依赖这层量出来的矩形 —— 没有层就量不到矩形、量不到就永远
> `falloff === 0`，光效一次都不会出现。**负责测量的节点必须无条件渲染。**
