...
> 当前版本：1.1.2-OS（bundleName `com.miha.harmony`）

## 更新日志

| 版本 | 主要内容 |
| --- | --- |
| **1.1.2-OS** | 新增「卡片沾色」与「高权限自定义主题色」开关；深色模式改为跟随系统 / 浅色 / 深色三选一；修复卡片按压变色、沾色后卡片变大、小按钮背后阴影 —— [完整说明](docs/1.1.2.md) |
...
> **未发布**：`1.1.3-OS` 的改动已经在 `main` 上（见 [`docs/1.1.3.md`](docs/1.1.3.md)）——
> 新增「主题色流光」开关与「涟漪粉」「流萤绿」两个浅色主题色、修复切换标签页后列表不回顶部等，
> 但版本号尚未提升、也还没有 tag，等正式发布时再补进上表。
