...
> 当前版本：1.1.4-OS（bundleName `com.miha.harmony`）

## 更新日志

| 版本 | 主要内容 |
| --- | --- |
| **1.1.4-OS** | 修好长期失效的「双列模式」：首页设备卡改两列网格（图标在左上角、快捷开关在右下角），骨架屏跟随列数；设备页双列拆成独立开关并默认关闭（标注「不实用」）；登录页右上角新增「跳过登录」与「外观设置」两个圆钮，跳过登录后按「未登录用户」显示并提供「登录」入口；首页顶栏新增「切换家庭」按钮并适配沉浸光感；选择家庭页卡片改用沉浸光感材质、去掉成排卡片的多余投影 —— [完整说明](docs/1.1.4.md) |
| [1.1.3-OS](docs/1.1.3.md) | 新增「主题色流光」与「卡片流光触感」开关，按压卡片的流光改用主题色并可跟手；新增「涟漪粉」「流萤绿」两个浅色主题色，主题色底上的字色按深浅自动取黑 / 白；修复切换标签页后列表不回顶部、更换主题色后底栏高亮不跟随、耗材中心文字看不清 —— [完整说明](docs/1.1.3.md) |
| [1.1.2-OS](docs/1.1.2.md) | 新增「卡片沾色」与「高权限自定义主题色」开关；深色模式改为跟随系统 / 浅色 / 深色三选一；修复卡片按压变色、沾色后卡片变大、小按钮背后阴影 —— [完整说明](docs/1.1.2.md) |
| [1.1.1-OS](docs/1.1.1.md) | 修复部分设备启动闪退、超大照片背景裁切不出图、折叠屏裁剪框比例、主题色与页面底色不同步等问题；卡片在沉浸光感不可用时降级为毛玻璃 —— [完整说明](docs/1.1.1.md) |
...
- **米家扫码登录**：基于 `serviceLogin` 的二维码登录流程，长期轮询票据，自动组装授权数据；登录页可跳过登录，以未登录状态浏览界面与调整外观。
- **家庭 / 房间 / 设备管理**：拉取家庭列表、房间与设备数据，支持设备在线 / 缓存 / 离线状态区分；首页顶栏可切换当前家庭或「全部家庭」。
...
- **外观设置**：自定义背景（含裁切）与字体管理，全局沉浸光感开关、主题色流光与卡片流光触感开关。
- **备份与恢复**：导出为 ZIP 压缩包，可选附带自定义背景与字体，恢复时同步还原资源，并按设备比对过滤不适用设置项。
- **反馈与日志**：完整日志模式，导出日志时对 Token、密钥等敏感参数脱敏；支持教程与选择保存位置。
- **悬浮底栏与主页优化**：主页平滑切换、隐藏离线设备实时生效、首页设备卡双列（两列网格，图标左上 / 快捷开关右下）、音箱播放 / 麦克风控制与音量阶梯调节。
...
| --- | --- |
| LoginPage | 米家二维码登录 |
| FamilySelectPage | 家庭选择（首页顶栏「切换家庭」入口，可切到某个家庭或「全部家庭」） |
...
- **成排的小卡片关掉材质自带投影**：传 `cardMaterialShadow: false`。材质仍照常渲染，只是不带
  `applyShadow` 那层投影——默认 `true` 时大卡片的投影是加分项，但成排小卡片挨得近，
  投影会在卡间缝隙里叠成一条暗带（选择家庭页就是这么改的）。取舍依据见
  [`docs/沉浸光感Toggle承载方案.md`](docs/沉浸光感Toggle承载方案.md) §8。
